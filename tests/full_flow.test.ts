...
import { getOnChainTimestamp } from "../src/rpc";
...
async function withdraw(presale: Presale, userKeypair: Keypair, amount: BN) {
  await presale.refetchState();
  let escrowState = await presale
    .getPresaleEscrowByOwner(userKeypair.publicKey)
    .then((escrows) => escrows[0]);
...
async function deposit(presale: Presale, userKeypair: Keypair, amount: BN) {
  await presale.refetchState();
...
async function createEscrowWithProof(
  connection: Connection,
  presale: Presale,
...
async function waitUntilTimestamp(connection: Connection, timestamp: number) {
  while (true) {
...
async function initializeMerkleRootConfigs(
  connection: Connection,
  presale: Presale,
  whitelistWallets: WhitelistedWallet[],
  userKeypair: Keypair
...
async function initPermissionedMerkleTreeFixedPricePresale(
  connection: Connection,
